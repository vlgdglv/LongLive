...
# ------------------------------ Probe ------------------------------------
_probe_cfg = getattr(config, "latent_probe", None)
_probe_enabled = bool(getattr(_probe_cfg, "enabled", False)) if _probe_cfg else False
if _probe_enabled and local_rank == 0:
    print(f"[Probing] Probe ENABLED...")
    latent_collector = LatentCollector(enabled=True, seed=config.seed)
...
for i, batch_data in tqdm(enumerate(dataloader), disable=(local_rank != 0)):
    ...
    video = pipeline.inference(
        ...
        latent_collector=latent_collector,
    )

    continue # No video decode
